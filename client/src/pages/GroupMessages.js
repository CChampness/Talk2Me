...
function GroupMessages ({ currentPage, handleChange }) {
  ...
  const { msgTgtName, msgTgtType } = useGlobalContext();
  ...
  console.log("in GroupMessages, msgTgtName, msgTgtType:", msgTgtName, msgTgtType);
    ...
    userData.map((msg, ndx) =>
      <div key={ndx} className="card-column">
        <figure className="proj-card">
          <p>Message from: {msg.messageFrom}
            <ReactNbsp/> on: {new Date(parseInt(msg.createdAt)).toLocaleDateString()}
            <ReactNbsp/>at: { new Date(parseInt(msg.createdAt)).toLocaleTimeString()}
          </p>
          <p>{msg.messageText}</p>
          <Button
            type='submit'
